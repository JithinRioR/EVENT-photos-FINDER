...
public class PhotoMatchingController {
    ...
    // Sync files and extract faces into local DB cache (Asynchronous)
    @PostMapping("/admin/sync")
    public ResponseEntity<Map<String, Object>> syncEventPhotos(
            @RequestParam("folderId") String folderId) {

        try {
            Map<String, Object> syncResult = photoMatchingService.startSync(folderId);
            return ResponseEntity.ok(syncResult);
        } catch (Exception e) {
            System.err.println("Sync trigger error: " + e.getMessage());
    ...
    // Check background sync status & progress
    @GetMapping("/admin/sync-status")
    public ResponseEntity<Map<String, Object>> getSyncStatus(
            @RequestParam("folderId") String folderId) {
        Map<String, Object> status = photoMatchingService.getSyncStatus(folderId);
